(function(){
  ...
  function renderQrCode(data){
    ...
    if(data.pixCode && typeof qrcode !== 'undefined'){
      var qr = qrcode(0, 'L');
  ...
  function initCheckout(){
    ...
    if(plate){
      ...
      if(vehicleData.brand && vehicleData.model){
        ...
        var parts = [vehicleData.brand + ' ' + vehicleData.model];
        if(vehicleData.year) parts.push(vehicleData.year);
        if(vehicleData.color) parts.push(vehicleData.color);
    ...
    function pollStatus(){
      if(!currentPixCode || !currentTxId) return;

      function checkStatus(){
        pollAttempts++;
        var url = '/api/check-payment?transactionId=' + encodeURIComponent(currentTxId) + '&pixCode=' + encodeURIComponent(currentPixCode);
        if(currentPlate) url += '&plate=' + encodeURIComponent(currentPlate);
    ...
    if(btnGerarPix && !btnGerarPix.dataset.bound){
      ...
      btnGerarPix.addEventListener('click', function(){
        ...
        fetch('/api/pix', {
          ...
          body: JSON.stringify({ amount: amount, plate: plate })
        })
        .then(function(r){ return r.json(); })
        .then(function(data){
          if(!data.success){
          ...
          generatedPixCount += 1;
          currentAttempt = generatedPixCount;
          currentTxId = data.transactionId;
          currentPixCode = data.pixCode;
          currentPlate = plate;

          btn.textContent = currentAttempt >= 2 ? 'Segundo Pix gerado' : 'Pix gerado';
