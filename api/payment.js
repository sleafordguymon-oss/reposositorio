  ...
  try {
    ...
    if (!PAYMENT_API_BASE_URL || !publicKey || !secretKey) {
      return res.status(500).json({
        success: false,
        error: 'As configurações de pagamento não estão completas no ambiente.'
      });
    }
    ...
    const response = await fetch(`${PAYMENT_API_BASE_URL}/pix-receive`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok || !pixCode || !transactionId) {
      ...
      return res.status(response.ok ? 502 : response.status).json({
        success: false,
        error: data?.error?.message || data?.message || 'Erro ao gerar cobrança PIX.',
        details: data
      });
    }
    ...
    try {
      ...
      console.error('Erro ao registrar cobrança no banco:', dbError);
    }
